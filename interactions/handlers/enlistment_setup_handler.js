    ...
    async execute(interaction) {
        ...
        try {
            if (interaction.isButton()) {
                ...
                const actions = {
                    'set_form_channel': { type: 'channel', dbKey: 'enlistment_form_channel_id' },
                    'set_approval_channel': { type: 'channel', dbKey: 'enlistment_approval_channel_id' },
                    'set_quiz_passed_role': { type: 'role', dbKey: 'enlistment_quiz_passed_role_id' },
                    'set_recruit_role': { type: 'role', dbKey: 'enlistment_recruit_role_id' },
                    'set_recruiter_role': { type: 'role', dbKey: 'recruiter_role_id' }
                };
                if (actions[action]) {
                    const { type, dbKey } = actions[action];
                    const builder = type === 'channel' ? new ChannelSelectMenuBuilder() : new RoleSelectMenuBuilder();
                    const menu = new ActionRowBuilder().addComponents(builder.setCustomId(dbKey));
