...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (member && role) await member.roles.add(role, `Certificado no curso: ${course.name}`);

            // Remove o membro da thread de discussão
            if (course.thread_id) {
        ...
        if (logChannelId) {
            ...
            if (logChannel) {
                const logEmbed = new EmbedBuilder()
                    .setColor('Green').setTitle('🎖️ Nova Certificação').setThumbnail(member.user.displayAvatarURL())
                    .addFields(
